...
화살표 버튼은 치트시트 Hard Rules(캐러셀/슬라이더/화살표 금지)에 따라 전체 페이지에서 사용하지 않았습니다.
다만 사용자가 명시적으로 지정한 두 곳에서는 예외적으로 슬라이드 인터랙션을 반영했습니다.

- **Showcase(INDUSTRY COVERAGE)**: "슬라이드로, 가운데 카드는 커지고 양옆은 작게"라는 요청에 따라, 화살표·점
  인디케이터 없이 네이티브 가로 스크롤/드래그 + 중앙 카드 확대 방식으로 구현.
- **Hero**: 레퍼런스 스크린샷에 실제로 점(dot) 페이지네이션 + 일시정지 아이콘이 있었고, 사용자가 "페이지네이션
  작동 안해? 알아서 넘어가야 하고 드래그하면 넘어가고"라고 명시적으로 요청해, 자동 전환(5초 간격) + 점 클릭
  이동 + 일시정지 토글 + 드래그/스와이프 전환을 갖춘 실제 동작하는 슬라이더로 구현. `prefers-reduced-motion`
  사용자에게는 자동 전환을 비활성화.
